...
$sql_stmt = 'SELECT * FROM ISS, USR WHERE ISS.ISS_USR_SPR_ID = '. $_SESSION['usr_id'] .' AND ISS.ISS_USR_CNCRN_ID=USR.USR_ID ORDER BY ISS.ISS_DT DESC, ISS.ISS_TM DESC';
if($sql = mysqli_query($con, $sql_stmt)) {
	$iter = 0;
	while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
