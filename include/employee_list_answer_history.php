...
if($sql = mysqli_query($con, $sql_stmt)) {
	$iter = 0;
	while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
		echo $row['ISS_CNCRN_APPRVL_STS'];
		if (empty($row['ISS_CNCRN_APPRVL_STS'])) {
			$apprvl_sts = '';
		} else {
			$apprvl_sts = $row['ISS_CNCRN_APPRVL_STS'];
		}
		/* if ($row['ISS_VLDT_STS'] == 'VALIDATED' || $row['ISS_CNCRN_APPRVL_STS'] == 'VALIDATED') {
			$sts = '<a style="color: blue;" href="'. HTTP_ROOT .'employee-respond-cpar.php?issue_id_to_answer='.$row['ISS_ID'].'">Respond</a>';
			$apprvl_sts = '';
		...
		if($row['ISS_CNCRN_VLDT_STS'] == 'VALIDATED') {
			$sts = 'Closed';
		} else if($row['ISS_CNCRN_APPRVL_STS'] == 'Denied' || !empty($row['ISS_CNCRN_RSN_FR_DNL'])) {
			$sts = 'Closed';
		} else if($row['ISS_CNCRN_VLDT_STS'] == 'VALIDATE') {
			$sts = 'Pending';
		} else {
			$sts = '<a style="color: blue;" href="'. HTTP_ROOT .'employee-respond-cpar.php?issue_id_to_answer='.$row['ISS_ID'].'">Respond</a>';
			$apprvl_sts = '';
